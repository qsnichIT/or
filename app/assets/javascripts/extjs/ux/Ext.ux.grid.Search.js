...
Ext.extend(Ext.ux.grid.Search, Ext.util.Observable, {
	...
	/**
	 * @cfg {Object} paramaterial_names Params name map (defaults to {fields:'fields', query:'query'}
	 */
	,paramaterial_names: {
		 fields:'fields'
		,query:'query'
	...
	,onTriggerSearch:function() {
		...
		// ask server to filter records
			...
			if(store.lastOptions && store.lastOptions.params) {
				store.lastOptions.params[store.paramaterial_names.start] = 0;
			}
			...
			delete(store.baseParams[this.paramaterial_names.fields]);
			delete(store.baseParams[this.paramaterial_names.query]);
			if (store.lastOptions && store.lastOptions.params) {
				delete(store.lastOptions.params[this.paramaterial_names.fields]);
				delete(store.lastOptions.params[this.paramaterial_names.query]);
			}
			if(fields.length) {
				store.baseParams[this.paramaterial_names.fields] = Ext.encode(fields);
				store.baseParams[this.paramaterial_names.query] = val;
			}
